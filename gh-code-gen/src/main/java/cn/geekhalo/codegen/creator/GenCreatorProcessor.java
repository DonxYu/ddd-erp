...
import cn.geekhalo.codegen.soaupdater.DataOptional;
...
public class GenCreatorProcessor extends BaseProcessor<GenCreator> {
    public GenCreatorProcessor() {
        super(GenCreator.class);
    }

    @Override
    protected void foreachClass(GenCreator genCreator, Element element, RoundEnvironment roundEnv) {
        String packageName = element.getEnclosingElement().toString() ;
        String className = "Base" + element.getSimpleName().toString() + "Creator";

        String parentClassName = getParentClassName(genCreator, element);
    ...
    private Set<TypeAndName> findPublicSetter(Element element) {
        ...
        Set<TypeAndName> getterMethodResult = getterMethods.stream()
                ...
                .map(element1 -> {
                    ...
                    GenCreatorIngore genVOIgnore = element1.getAnnotation(GenCreatorIngore.class);
    ...
    private FieldConfig toFieldConfig(VariableElement element){
        ...
        boolean ignore = element.getAnnotation(GenCreatorIngore.class) != null;
        Description description = element.getAnnotation(Description.class);
        String descr = description == null ? "" : description.value();
        return new FieldConfig(name, ignore, descr);
    }

    private String getParentClassName(GenCreator genCreator, Element element) {
        String parent = genCreator.parent();
        ...
        if (element instanceof TypeElement){
            ...
            if (Object.class.getName().equals(superClass)){
                ...
                return getPackageName(typeElement.getSuperclass().toString()) + ".Base" + getSuperClassName(typeElement.getSuperclass().toString()) +"Creator";
            }
            else {
...
//        return pkgName + ".updater.Base" + clsName + "Updater";
        return "cn.geekhalo.codegen.creator.BaseAbstractEntityCreator";
    }
